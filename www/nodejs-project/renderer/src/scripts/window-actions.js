...
class WindowActions extends EventEmitter {
	...
	canAutoRestart(){ 
		...
		} else if(typeof(cordova) != 'undefined' && cordova && parseInt(device.version) < 10 && typeof(plugins) != 'undefined' && plugins.megacubo) {
			autoRestartSupport = true
		}
...
class WinActionsMiniplayer extends WindowActions {
	...
	getDimensions(real){
		const scr = window.cordova ? screen : parent.Manager.getScreenSize(real), streamer = this.getStreamer()
...
export class CordovaWinActions extends WinActionsMiniplayer {
    constructor(main){
		super(main)
		...
		this.setup()
		this.on('enter', () => {
			document.body.classList.add('miniplayer-cordova')
		})
		this.on('leave', () => {
			this.enteredPipTimeMs = false
			console.warn('leaved miniplayer')
			document.body.classList.remove('miniplayer-cordova')
			if(window.streamer && streamer.active){
				streamer.enterFullScreen()	
			}
			if(this.appPaused){
