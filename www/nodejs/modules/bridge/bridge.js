...
class AndroidChannel extends BaseChannel {
    ...
    attach(){
        if(!this.channel && global.paths.android.channel){
            this.channel = global.paths.android.channel
...
class BridgeServer extends EventEmitter {
    constructor(opts){
        ...
        this.server = http.createServer((req, response) => {
            if(!this.checkUA(req.headers)) {
                response.writeHead(400, global.prepareCORS({'content-type': 'text/plain'}, req))
                return response.end()
            }
            ...
            if(parsedUrl.pathname == '/upload') {
                ...
                form.parse(req, (err, fields, files) => {
                    response.writeHead(200, global.prepareCORS({'content-type': 'text/plain'}, req))
                    response.end('OK')
    ...
    checkUA(headers){
        if(global.paths.android) return true
