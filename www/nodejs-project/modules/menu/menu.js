...
class Menu extends EventEmitter {
	constructor(opts){
        ...
        global.renderer.on('menu-menu-playing', showing => {
            const streamer = require('../streamer/main')
            if(streamer.active){
                if(showing) {
                    this.softRefreshLimiter.limiter.resume()
        ...
        global.renderer.on('menu-open', (path, tabindex) => {
            if(this.opts.debug){
                console.log('menu-open', path, tabindex)
            }
            this.open(path, tabindex).catch(global.displayErr)
        })
        global.renderer.on('menu-action', (path, tabindex) => this.action(path, tabindex))
        global.renderer.on('menu-back', () => this.back())
        global.renderer.on('menu-check', (path, val) => this.check(path, val))
        global.renderer.on('menu-input', (path, val) => {
            if(this.opts.debug){
                console.log('menu-input', path, val)
            }
            this.input(path, val)
        })
        global.renderer.on('menu-select', (path, tabindex) => {
    ...
    async updateHomeFilters(){
        const es = await this.applyFilters(this.pages[''], '')
        this.pages[''] = es
    ...
    dialog(opts, def, mandatory){
        return new Promise((resolve, reject) => {
            let uid = 'ac-'+ Date.now()
            global.renderer.once(uid, ret => resolve(ret))
            global.renderer.emit('dialog', opts, uid, def, mandatory)
        })
    }
    prompt(atts){
        return new Promise((resolve, reject) => {
            ...
            global.renderer.once(atts.callback, ret => resolve(ret))
            global.renderer.emit('prompt', atts)
        })
    }
    info(question, message, fa){
        global.renderer.emit('info', question, message, fa)
    }
    checkFlags(entries){
	...
	setLoadingEntries(es, state, txt){
		...
		}).forEach(e => global.renderer.emit('set-loading', e, state, txt))
	}
    basename(path){
    ...
    async select(destPath, tabindex){
        ...
        if(ret && ret != -1){
            if(ret.entries && ret.entries.length > 1){
                ...
                global.renderer.emit('menu-select', ret.entries, destPath, icon)
            } else {
    ...
    chooseFile(mimeTypes='*') {
        return new Promise((resolve, reject) => {
            ...
            global.renderer.once(id, data => {
                if(data == null) return reject('File not selected')
                global.renderer.resolveFileFromClient(data).then(resolve).catch(reject)
            })
            global.renderer.emit('open-file', global.renderer.uploadURL, id, mimeTypes)
        })
    }
}
