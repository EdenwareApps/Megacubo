...
var torrentsNotification = notify('...', 'fa-magnet', 'forever');
    ...
    self.run = () => {
        if(Config.get('p2p')){
            ...
            self.peerflix.server.on('listening', () => {
                self.streamURL = 'http://127.0.0.1:' +  self.peerflix.server.address().port + '/';
                self.stream()
    ...
    self.stream = () => {
        ...
        if(!self.streamStarted){
            ...
            if(self.transcode){
                ...
                self.decoder.file = 'stream/' + self.uid + '/output.m3u8';    
