...
function createDirectIntent(entry, options){
    ...
    self.runConfirm = () => {
        ...
        if(typeof(iprx) == 'object' && iprx.isSupported(self.streamURL)){
            console.warn('BEFORE IPRX', self.streamURL)
            self.streamURL = iprx.resolve(self.streamURL)
            console.warn('AFTER IPRX', self.streamURL)
        }
        self.resetTimeout()
        self.test((worked) => {
            if(worked){
