...
function createTSIntent(entry, options){
    ...
    var file = path.resolve(self.workDir + path.sep + self.uid + path.sep + 'output.m3u8')
...
function getTSStreamer(){
    if(!TSStreamerInstance){
        var debug = debugAllow(false), connectionDurationSecs = 5, ids = 0, port = 0, request = prepareRequestForever()
