...
export const loadJS = (url, cb, retries=3) => {
...
export const loadJSOnIdle = (url, cb, retries=3) => {
    requestIdleCallback(() => {
        loadJS(url, (...args) => {
...
export const prepareFilename = (file, keepAccents) => {
...
export const centralizedResizeWindow = (w, h) => {
    var tw = window.top;
    if(tw){
...
export const isValidPath = (url) => { // poor checking for now
...
export const getManifest = (callback) => {
...
export const wordWrapPhrase = (str, count, sep) => {
...
export const isTS = (url) => {
    if(typeof(url)!='string') return false;
    return ['m2ts', 'ts'].indexOf(getExt(url)) != -1  
}

export const isRemoteTS = (url) => {
    if(typeof(url)!='string') return false;
...
export const checkPermissions = (_perms, callback) => {
	if(!Array.isArray(_perms)){
		_perms = [_perms]
	}
	_perms = _perms.map(p => {
		if(typeof(p) == 'string'){
			p = window.cordova.plugins.permissions[p]
		}
		return p
	})
	window.cordova.plugins.permissions.checkPermission(_perms, status => {
		console.log('checking permissions => '+ JSON.stringify({_perms, status}))
		if (status.hasPermission) {
			return callback(true)
		}
        window.cordova.plugins.permissions.requestPermissions(_perms, status => callback(!!status.hasPermission), () => callback(false))
	}, null)
}

export const isYoutubeURL = (source) => {
...
export const setupFontDetector = () => {
