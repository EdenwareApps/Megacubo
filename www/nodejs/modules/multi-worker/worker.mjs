...
parentPort.on('message', msg => {
    if(msg.method == 'configChange'){
        ...
        if(fs.existsSync(distFile)) {
            ...
            console.error("::::::::::: DRIVER LOADED "+ msg.file +" - "+ Object.keys(Driver).join(','))
            ...
            console.error("::::::::::: DRIVER LOAD... "+ msg.file.replace(new RegExp('\\\\', 'g'), '/'))
            import(msg.file.replace(new RegExp('\\\\', 'g'), '/')).then(Driver => {
                console.error("::::::::::: DRIVER LOADED "+ msg.file +" - "+ Object.keys(Driver).join(','))
                drivers[msg.file] = new Driver.default()
                if(typeof(drivers[msg.file].terminate) != 'function') {
                    console.error('Warning: worker '+ msg.file +' has no terminate() method.')
                }        
            }).catch(err => {
                console.error("::::::::::: DRIVER LOAD ERR "+ msg.file +" - "+ err)
            })
        }
