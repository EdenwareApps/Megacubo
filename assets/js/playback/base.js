...
var Playback = (() => {
    ...
    self.notification = notify('...', 'fa-check', 'forever');
    ...
    self.bind = (video, intent) => {
        ...
        var nativeEvents = {
            ...
            'volumechange': () => {
                if(self.active){
                    ...
                    Store.set('volume', video.volume)
                    Store.set('muted', video.muted)
                    if(self.active){
        ...
        var lastWidth, timer = 0, f = () => {
            var currentWidth = jvideo.width();
            if(currentWidth != lastWidth && top && scope && scope.clearTimeout){
                ...
                timer = scope.setTimeout(() => {
                    self.lastVideoObserver.disconnect(); // ensure prevent looping
                    scope.clearTimeout(timer);
                    fitPlayerControlsToMenu(scope);
                    self.lastVideoObserver.observe(video)
                }, 200)
            }
        }
        applyCSSTemplate('assets/css/player.src.css', getPlayerScope())
        if(self.lastVideoObserver){
            self.lastVideoObserver.disconnect()
        }
        self.lastVideoObserver = new ResizeObserver(f);
        self.lastVideoObserver.observe(video);
        jQuery(scope).off('load resize').on('load resize', f);
    ...
    self.commitIntent = (intent) => {
        ...
        if(intent && self.active != intent){
            ...
            */
            ...
            self.lastActiveIntent = self.active = intent;
...
function createBaseIntent(){
    ...
    self.uid = parseInt(Math.random() * 1000000000), files = fs.readdirSync('stream');
