...
class M3UExtParser {
	...
	preSanitizeGroupsList(s){
		s = s.replaceAll('\\', '/').replace(new RegExp('(\\\\| \\| )', 'g'), '/').replace(new RegExp('( +\/|\/ +)', 'g'), '/').replace(new RegExp('(^[ \/]+|[ \/]+$)', 'gm'), '').replace(this.regexes['nullgroup'], '')
		return s
	}
	sanitizeGroupsList(s){
		s = s.replace(this.regexes['plus-signal'], 'plus').
	...
	parse(content){
		...
		const map = {'tvg-logo': 'icon', 'group-title': 'group', 'tvg-name': 'name', 'tvg-id': 'gid', 'pltv-subgroup': 'sub-group'}
		const r = new RegExp('('+ Object.keys(map).join('|').replace('-', '\-') +')\s*=\s*[\'"]([^\r\n\'"]+)', 'g')
		const matches = content.matchAll(new RegExp('(^(#[^\r\n]+),\s*([^\r\n]*)\s*[\r\n]+\s*([^#\r\n ]*)$)', 'gm'))
		let i = 0, entries = [], names = [], groups = [], rawGroups = []
		for(const match of matches){
			if(match[4].indexOf('/') != -1){
				let n = match[3], g = '', sg = '', e = {url: match[4].trim()}
				for(const t of match[2].matchAll(r)){
					if(t && t[2]){
						if(map[t[1]] == 'name'){
							if(!n || n == 'N/A'){
								n = t[2]
							}
						} else if(map[t[1]] == 'group'){
							if(!g || g == 'N/A'){
								g = t[2]
							}
						} else if(map[t[1]] == 'sub-group'){
							if(!sg || sg == 'N/A'){
								sg = t[2]
							}
						} else if(!e[map[t[1]]]){
		...
		//console.log('parsing', names, groups)
		names = this.sanitizeNamesList(names.join("\n")).split("\n")
		rawGroups = this.preSanitizeGroupsList(groups.join("\n")).split("\n")
		groups = this.sanitizeGroupsList(rawGroups.join("\n")).split("\n")
		//console.log('parsing', names, groups, rawGroups)
		for(let i = 0; i < entries.length; i++){
			if(entries[i].url.substr(0, 2) == '//'){
				entries[i].url = 'http:' + entries[i].url
			}
			entries[i].name = names[i]
			entries[i].group = groups[i]
			entries[i].groups = groups[i].split('/')
			entries[i].groupName = rawGroups[i].split('/')
			entries[i].groupName = entries[i].groupName.pop()
			if(typeof(entries[i]['icon']) == 'undefined'){
				entries[i]['icon'] = ''
			}
		}
		entries = entries.filter(e => {
			return this.validateURL(e.url)
		})
