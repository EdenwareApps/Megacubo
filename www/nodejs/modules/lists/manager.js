...
class Manager extends ManagerEPG {
    ...
    async addList(listUrl, name, fromCommunity){
        ...
        if(typeof(err) != 'undefined'){
            ...
            if(!global.ALLOW_ADDING_LISTS) {
                ...
                global.menu.info(lang.LEGAL_NOTICE, lang.TOS_CONTENT)
            }
    ...
    async directListRenderer(data, opts={}){
        ...
        opts.silent || global.osd.show(global.lang.OPENING_LIST, 'fa-mega spin-x-alt', 'list-open', 'persistent')
        let list = await this.master.directListRenderer(v, {
            fetch: opts.fetch,
            expand: opts.expand,
            progress: p => {
                opts.silent || global.osd.show(global.lang.OPENING_LIST +' '+ parseInt(p) +'%', 'fa-mega spin-x-alt', 'list-open', 'persistent')
            }
        ...
        this.openingList = false
        opts.silent || global.osd.hide('list-open')
        return list
    }
