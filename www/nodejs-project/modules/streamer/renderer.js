...
class StreamerSeek extends StreamerButtonActionFeedback {
    ...
    seekTo(_s, type){
        ...
        if(window.cordova && this.activeMimetype == 'video/mp2t') {
...
class StreamerClientVideoFullScreen extends StreamerAndroidNetworkIP {
    ...
    updateAndroidAppMetrics(metrics){
        if(this.inFullScreen){
            css(' :root { --menu-padding-top: 0px; --menu-padding-bottom: 0px; --menu-padding-right: 0px; --menu-padding-left: 0px; } ', 'frameless-window')
        } else {
            ...
            if(metrics) {
                css(' :root { --menu-padding-top: ' + metrics.top + 'px; --menu-padding-bottom: ' + metrics.bottom + 'px; --menu-padding-right: ' + metrics.right + 'px; --menu-padding-left: ' + metrics.left + 'px; } ', 'frameless-window')
            }
        }
    }
...
class StreamerAudioUI extends StreamerClientVideoFullScreen {
    ...
    setupVolume(){        
        ...
        let isTouchDevice, touchListener = event => {
            isTouchDevice = true
            this.volumeInput.removeListener('touchstart', touchListener)
        }
...
class StreamerClientControls extends StreamerAudioUI {
    constructor(controls){
        ...
        if(!window.cordova && main.config['startup-window'] != 'fullscreen'){
