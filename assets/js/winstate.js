...
function restoreWindowState() {
    ...
    for (var i = 0; i < screens.length; i++) {
        ...
        if (winState.x > (screen.bounds.x - offsetTolerance) && winState.x < (screen.bounds.x + screen.bounds.width + (offsetTolerance * 2))) {
            if (winState.y > (screen.bounds.y - offsetTolerance) && winState.y < (screen.bounds.y + screen.bounds.height + (offsetTolerance * 2))) {
                console.log("[winstate] Location of window (" + winState.x + "," + winState.y + ", " + winState.width + "x" + winState.height + ") is on screen " + JSON.stringify(screen));
    ...
    if (!locationIsOnAScreen) {
        console.log("[winstate] Last saved position of windows is not usable on current monitor setup. Moving window to center!", screen.bounds, winState);
        win.setPosition("center");
    }
    else {
...
addAction('beforeRestartApp', function () {
    try {
        saveWindowState();
    } catch(err) {
        console.log("[winstate] winstateError: " + err);
    }
});

addAction('beforeTray', function () {
    try {
        saveWindowState();
    } catch(err) {
        console.log("[winstate] winstateError: " + err);
...
addAction('appUnload', function () {
    if(isMiniPlayerActive() || !$tray){
        try {
            saveWindowState();
        } catch(err) {
            console.log("[winstate] winstateError: " + err);
