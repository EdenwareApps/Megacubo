function patch(scope) {
      ...
      value: function (p, reverse) {
        if (Array.isArray(this)) {
          ...
          return this.slice(0).sort((a, b) => {
            let ua = typeof a[p] == 'undefined',
              ub = typeof b[p] == 'undefined';
            if (ua && ub) return 0;
  ...
  scope.kbsfmt = (bytes, decimals = 1) => {
    // https://stackoverflow.com/questions/15900485/correct-way-to-convert-size-in-bytes-to-kb-mb-gb-in-javascript
    if (isNaN(bytes) || typeof bytes != 'number') return 'N/A';
    if (bytes === 0) return '0 Bytes/ps';
    const k = 1024,
      dm = decimals < 0 ? 0 : decimals,
      sizes = ['Bytes/ps', 'KBps', 'MBps', 'GBps', 'TBps', 'PBps', 'EBps', 'ZBps', 'YBps'];
    const i = Math.floor(Math.log(bytes) / Math.log(k));
    return parseFloat((bytes / Math.pow(k, i)).toFixed(dm)) + ' ' + sizes[i];
  };
  ...
  scope.hexToRgb = ohex => {
    var shorthandRegex = /^#?([a-f\d])([a-f\d])([a-f\d])$/i,
      hex = ohex.replace(shorthandRegex, (m, r, g, b) => {
        return r + r + g + g + b + b;
      });
  ...
  scope.isVODM3U8 = (content, contentLength) => {
    ...
    if (sample.indexOf('#ext-x-program-date-time') == -1) {
      ...
      if (typeof contentLength == 'number' && pieces.length > 2) {
        //  at least 3 pieces, to ensure that the first extinf is complete
  ...
  scope.networkIP = () => {
    let interfaces = scope.networkInterfaces(),
      addr = '127.0.0.1',
      skipIfs = new RegExp('(vmware|virtualbox)', 'i');
  ...
  scope.parseJSON = json => {
    // prevent JSON related crashes
    let ret;
    try {
